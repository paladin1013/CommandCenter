classdef ReferenceField < Prefs.Inputs.LabelControlBasic
    ...
    methods
        function [obj, height_px, label_width_px] = make_UI(obj, pref, parent, yloc_px, width_px, margin_px, readonly)
            ...
            msm = pref.parent.parent; % MetaStageManger;
            ...
            if (isempty(obj.optimize) || ~isvalid(obj.optimize) || ~isprop(obj.optimize, 'UserData') || isempty(obj.optimize.UserData))
                if strcmp(pref.name, "Target")
                    obj.optimize = uicontrol(parent,...
                                    'Style', 'togglebutton',...
                                    'String', char(0x2605),...  % Gear icon 0x2699, tool icon 0x2692
                                    'FontSize', 11, ...
                                    'HorizontalAlignment','center',...
                                    'Units', 'pixels',...
                                    'Tag', tag,...
                                    'Enable', 'on',...
                                    'Tooltip', 'Optimize globally (through all loaded axes). It will automatically release after finishing optimization, or user can click again to abort optimization.',...
                                    'Callback', @(src, event) msm.global_optimize_Callback(src, event),...
                                    'Enable', enable, ...
                                    'UserData', reference);
                elseif isprop(reference, "steponly") && ~isempty(reference) && reference.steponly
                    obj.optimize = uicontrol(parent,...
                                    'Style', 'togglebutton',...
                                    'String', char(0x26e4),...  % Gear icon 0x2699, tool icon 0x2692
                                    'FontSize', 11, ...
                                    'HorizontalAlignment','center',...
                                    'Units', 'pixels',...
                                    'Tag', tag,...
                                    'Enable', 'on',...
                                    'Tooltip', 'Optimize the step-only value. It will automatically release after finishing optimization, or user can click again to abort optimization.',...
                                    'Callback', @(src, event) msm.steponly_optimize_Callback(src, event, pref.name),...
                                    'Enable', enable, ...
                                    'UserData', reference);
                else % The global optimize button set to target reference panel.
                    obj.optimize = uicontrol(parent,...
                                    'Style', 'togglebutton',...
                                    'String', char(0x2606),...  % Gear icon 0x2699, tool icon 0x2692
                                    'FontSize', 11, ...
                                    'HorizontalAlignment','center',...
                                    'Units', 'pixels',...
                                    'Tag', tag,...
                                    'Enable', 'on',...
                                    'Tooltip', 'Optimize this axis to get higher target value. It will automatically release after finishing optimization, or user can click again to abort optimization.',...
                                    'Callback', @(src, event) msm.optimize_Callback(src, event, pref.name),...
                                    'Enable', enable, ...
                                    'UserData', reference);
                end
            end
