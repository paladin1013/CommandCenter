...
% freqs = 484.12246:(484.15088-484.12246)/500:484.15088;
...
%     colormap('bone')
%     imagesc(imgs(:,:,i));
%     tt=strcat(num2str(i),' Freq:',num2str(d.data.data.data.freqMeasured(i),'%4.6f'),'THz; Voltage:',num2str(100*(i-1)/(length(freqs)-1)),'V');
%     title(tt);
%     waitforbuttonpress
% end
...
for i = 1:length(fres)
    ...
    if (realx(i) < rxmax) & (realx(i) > rxmin) & (realy(i) < rymax) & (realy(i) > rymin)
        ...
        if max(wgt(max(1, wgtp - floor(length(wgt) / 20)):min(length(wgt), wgtp + floor(length(wgt) / 20)))) > 0.5 * max(yy(i, :))
            %         plot((freqs-min(freqs)*ones(1,length(freqs)))*1e3,yy(i+12*0,:),'linewidth',2,'Color',c(length(fres)+1-i,:))
