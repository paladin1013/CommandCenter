...
fitBox = [1 1]*NVsize*2;
fitBox = round(fitBox/2)*2;  % Make sure even integer
...
im_filt = imgaussfilt(im,NVsize*3/8) - imgaussfilt(im,NVsize*3);  % BP filter: LP-HP
if nargin == 5 && ishandle(ax) && isvalid(ax)
    hold(ax,'off');
    imagesc(im_filt,'parent',ax);
    axis(ax,'image');
    set(ax,'ydir','normal')
    colormap(ax,'gray');
    hold(ax,'on');
...
edge = max(fitBox);
candidates = FastPeakFind(im_filt(edge:end-edge,edge:end-edge),thresh);
...
candidates = round(candidates)+edge-1;
