classdef GalvoVoltage < Modules.Imaging
    ...
    methods(Access=private)
        function obj = GalvoVoltage()
            obj.uses_stage = 'Stages.Galvos';
            obj.path = 'APD1';
            obj.loadPrefs;
            if obj.use_z
                try
                    obj.galvos = Drivers.NIDAQ.stage.instance('X','Y','Z','Vreadout','GalvoScanSync');
                catch err
                    if ~isempty(strfind(err.message,'No line with name "Z".'))
                        ...
                        if strcmp(answer,'yes')
                            obj.galvos = Drivers.NIDAQ.stage.instance('X','Y','','Vreadout','GalvoScanSync');
                            obj.use_z = false;
                        else
                            rethrow(err)
                        end
    ...
    methods(Static)
        function obj = instance()
            ...
            if isempty(Object) || ~isvalid(Object)
                Object = Imaging.Confocal.GalvoVoltage();
            end
            obj = Object;
        end
    end
    methods
        ...
        function snap(obj,im,continuous)
            ...
            obj.galvos.SetupScanVoltage(x,y,obj.dwell)
            if ~continuous
            ...
            obj.galvos.StartScanVoltage;
            try
                obj.galvos.StreamToImageVoltage(im);
            catch err
            end
            if ~continuous
                delete(h);
