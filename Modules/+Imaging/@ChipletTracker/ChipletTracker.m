classdef ChipletTracker < Modules.Imaging
    ...
    methods
        ...
        function delete(obj)
            ...
            timer = timerfind('Name', 'ChipletTracker Video');
            if ~isempty(timer)
                stop(timer);
                delete(timer);
            end
            ...
            if ~exist('hImage', 'var') || ~isvalid(hImage)
                if isempty(obj.hImage) || ~isvalid(obj.hImage)
                    if isempty(obj.camera.hImage) || ~isvalid(obj.camera.hImage)
                        error('Please click `Continuous` in image panel to initialize obj.hImage');
                    else
        ...
        function startVideo(obj,hImage)
            ...
            obj.videoTimer = timer('Name', 'ChipletTracker Video', 'tag','Video Timer',...
