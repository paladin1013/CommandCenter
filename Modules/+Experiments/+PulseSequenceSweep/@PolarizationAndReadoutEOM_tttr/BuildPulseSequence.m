...
r_s1_begin_sync = node(r_s1,SyncChannel,'units','us', 'delta',0);
test = node(r_s1_begin_sync,SyncChannel,'units','us', 'delta',obj.SyncPulseWidth_us);

for k = 1:10
    test = node(test, SyncChannel,'units','us', 'delta', k/2-obj.SyncPulseWidth_us);
...
% r_s1_end_sync = node(r_s1_begin_sync,SyncChannel,'units','us', 'delta',obj.tauTimes(tauIndex));
...
% r_s2_begin_sync = node(r_s1_end_sync,SyncChannel,'units','us', 'delta',obj.readoutPulseDelay_us);
...
% r_s2_end_sync = node(r_s2_begin_sync,SyncChannel,'units','us', 'delta',obj.readoutPulseTime_us);
